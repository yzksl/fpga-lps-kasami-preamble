...
# Hardcoded Kasami Sequences (Generated previously)
...
class SignalGenerator:
    def __init__(self, samples=4096, sig_amp=50, noise_amp=5):
        ...
        self.mask = [0] * samples # For visualization
    ...
    def generate(self, end_a, end_b):
        ...
        # Inject Sequences
        self.inject(KASAMI_SEQ_A, end_a, 1) # Mask 1 = A
        self.inject(KASAMI_SEQ_B, end_b, 2) # Mask 2 = B
        
        # Add Noise & Convert to Bytes
        byte_data = bytearray()
        for s in self.buffer:
            noise = int(random.gauss(0, self.noise_amp))
            val = s + noise
            # Clip to signed 8-bit (-128 to 127)
            if val > 127: val = 127
            if val < -128: val = -128
            
            # Convert to unsigned byte for transmission (2's complement logic)
...
def unpack_scores(rx_bytes):
    ...
    for i in range(0, len(rx_bytes), BYTES_PER_FRAME):
        ...
        # Reconstruct 40-bit integer (Little Endian)
...
def main():
    ...
    # --- SERIAL TRANSACTION ---
    try:
        with serial.Serial(port, DEFAULT_BAUD, timeout=5) as ser:
            # 1. SEND
            ...
            # 2. WAIT FOR FPGA PROCESS
            print(f"\n{C_CYAN}--- FPGA INTERVENTION REQUIRED ---{C_RESET}")
            print("1. Flip 'SW_START' ON to process.")
            print("2. Wait for LEDs/Display to finish.")
            print("3. FPGA should automatically send data back.")
            input(f"{C_BOLD}Press ENTER immediately when FPGA starts uploading...{C_RESET}")
            ...
            if len(rx_data) != expected_bytes:
                print(f"{C_RED}Error: Incomplete Read! Got {len(rx_data)}/{expected_bytes} bytes.{C_RESET}")
                retry = input("Try reading remaining bytes? (y/n): ")
                if retry.lower() == 'y':
                    rx_data += ser.read(expected_bytes - len(rx_data))
